...
def readAndWrite(bbx_gttxtPath, dir_path):
    cnt = 0
    with open(bbx_gttxtPath, 'r') as f:
        ...
        for line in f:
            ...
            if len(inp)==1:
                ...
                if curr_img.isdigit():
                    continue
                # print(dir_path+'/'+curr_img)
                img = cv2.imread(dir_path + '/' + curr_img, 2) # POSIX only
                # print( len(list(curr_et_object.getroot()) )  )
                curr_filename = curr_img.split("/")[1].strip()
                curr_path = os.path.join(dir_path, os.path.dirname(curr_img))
...
def main(data_root):
    # ################################ TRAINING DATA ##################################
    train_path = os.path.join(data_root, "WIDER_train", "images" )
    ## comment this out
    train_bbx_gttxtPath = os.path.join(data_root, "wider_face_split", "wider_face_train_bbx_gt.txt" )
    readAndWrite(train_bbx_gttxtPath, train_path)

    # To folders:
    train_to_xml_folder = os.path.join(data_root, "tf_wider_train", "annotations", "xmls" )
    train_to_image_folder = os.path.join(data_root, "tf_wider_train", "images" )

    # make dir => wider_data in folder
    try:
        os.makedirs(train_to_xml_folder)
        os.makedirs(train_to_image_folder)
    ...
    train_annotations_index = os.path.join(data_root, "tf_wider_train", "annotations", "train.txt" )

    with open(train_annotations_index, "a") as indexFile:
        for f in file_list:
            if ".xml" in f:
                print(f)
                copyfile(f, os.path.join(train_to_xml_folder, os.path.basename(f) ))
                img = f.replace(".xml",".jpg")
                copyfile(img, os.path.join(train_to_image_folder, os.path.basename(img) ))
    ...
    val_path = os.path.join(data_root, "WIDER_val", "images" )
    val_bbx_gttxtPath = os.path.join(data_root, "wider_face_split", "wider_face_val_bbx_gt.txt" )
    readAndWrite(val_bbx_gttxtPath, val_path)


    # To folders:
    val_to_xml_folder = os.path.join(data_root, "tf_wider_val", "annotations", "xmls" )
    val_to_image_folder = os.path.join(data_root, "tf_wider_val", "images" )

    # make dir => wider_data in folder
    try:
        os.makedirs(val_to_xml_folder)
        os.makedirs(val_to_image_folder)
    except Exception as e:
        pass


    rootdir_glob = val_path + '/**/*' # Note the added asterisks # This will return absolute paths
    file_list = [f for f in iglob(rootdir_glob, recursive=True) if os.path.isfile(f)]

    train_annotations_index = os.path.join(data_root, "tf_wider_val", "annotations", "val.txt" )

    with open(train_annotations_index, "a") as indexFile:
        for f in file_list:
            if ".xml" in f:
                print(f)
                copyfile(f, os.path.join(val_to_xml_folder, os.path.basename(f) ))
                img = f.replace(".xml",".jpg")
                copyfile(img, os.path.join(val_to_image_folder, os.path.basename(img) ))
                indexFile.write(os.path.basename(f.replace(".xml","")) + "\n")



if __name__ == "__main__":
    data_root = '/home/purna/myworkspace/python/tensorflow-2/face-detector/data'
    main(data_root)
